...
def get_min_size(sprite):
    if hasattr(sprite, "get_min_size"):
        ...
        min_width, min_height = getattr(sprite, "width", 0) or 0, getattr(sprite, "height", 0) or 0
...
class Container(Widget):
    ...
    def __init__(self, contents = None, **kwargs):
        ...
        self._scene_on_frame_handler = None
    ...
    def __setattr__(self, name, val):
        ...
        if name == "parent" and val and self._scene_on_frame_handler is None and val == self.get_scene():
            self._scene_on_frame_handler = val.connect("on-frame", self._on_scene_frame)
    ...
    def _on_scene_frame(self, scene):
        # partial redraws query for new extents before the expose. do all the
        # updates here too (code cloned from the on_render event)
        if self._scene_on_frame_handler and not self.get_scene():
            # disconnect when we lose scene
            scene.disconnect(self._scene_on_frame_handler)
            self._scene_on_frame_handler = None
            return
        ...
        for sprite in self.sprites:
            if hasattr(sprite, "_on_scene_frame"):
                sprite._on_scene_frame(self)
    ...
    def get_min_size(self):
        ...
        width, height = self._cached_w, self._cached_h
        if width is None or height is None:
            ...
            width, height = max(width, self.min_width or 0), max(height, self.min_height or 0)
            self._cached_w, self._cached_h = width, height
        ...
        if w is None or h is None:
            ...
            for sprite in sprites:
                if hasattr(sprite, "get_min_size"):
                    ...
                    w, h = getattr(sprite, "width", 0) or 0, getattr(sprite, "height", 0) or 0
