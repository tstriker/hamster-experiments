...
class _DisplayLabel(graphics.Label):
    ...
    def get_min_size(self):
        _cached_w, _cached_h = self._cached_w, self._cached_h
        if _cached_w is not None and _cached_h is not None:
            return _cached_w, _cached_h

        text = self.markup or self.text
        escape = len(self.markup) == 0

        if self.wrap is not None or self.ellipsize is not None:
            _cached_w = self.measure(text, escape, 1)[0]
            _cached_h = self.measure(text, escape, -1)[1]
        else:
            _cached_w, _cached_h = self.measure(text, escape, -1)

        self._cached_w, self._cached_h = _cached_w, _cached_h
        return _cached_w, _cached_h

    def get_weight_for_width_size(self):
        cached_wh_w, cached_wh_h = self._cached_wh_w, self._cached_wh_h
        if cached_wh_w is not None and cached_wh_h is not None:
            return cached_wh_w, cached_wh_h

        text = self.markup or self.text
        escape = len(self.markup) == 0
        cached_wh_w, cached_wh_h = self.measure(text, escape, self.max_width)
        self._cached_wh_w, self._cached_wh_h = cached_wh_w, cached_wh_h
