...
class Widget(graphics.Sprite):
    ...
    def __setattr__(self, name, val):
        ...
        if name == "width":
            ...
            name = '_enabled'
        elif name in ("padding", "margin"):
            val = val or 0
            if isinstance(val, int):
                val = (val, )

            if len(val) == 1: # same size on all sides
                val = [val[0]] * 4
            elif len(val) == 2: # vertical, horizontal
                val = [val[0], val[1], val[0], val[1]]
            elif len(val) == 3: # top, horizontal, bottom
                val = [val[0], val[1], val[2], val[1]]
            else: # each side gets it's own size
                val = val[:4]

            self.__dict__['%s_top' % name]    = val[0]
            self.__dict__['%s_right' % name]  = val[1]
            self.__dict__['%s_bottom' % name] = val[2]
            self.__dict__['%s_left' % name]   = val[3]
            self.queue_resize()
            return
